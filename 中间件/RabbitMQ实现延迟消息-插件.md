...
> RabbitMQ实现延迟消息的方式有两种，一种是使用`死信队列`实现，另一种是使用`延迟插件`实现。`死信队列`实现我们以前曾经讲过，具体参考(RabbitMQ实现延迟消息-死信队列.md)，这次我们讲个更简单的，使用`延迟插件`实现。
...
​	![](../images/rabbitmq_28.png)

- 将插件文件复制到RabbitMQ安装目录的`plugins`目录下；

![](../images/rabbitmq_29.png)
